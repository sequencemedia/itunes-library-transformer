...
const transformCompilationAlbumsToM3U8 = (destination, ext) => ({ [TRACKS]: tracks }) => {
  if (tracks) {
...
const transformAlbumsToM3U8 = (destination, ext) => ({ [TRACKS]: tracks }) => {
  if (tracks) {
    const values = Array.from(tracks.values())
...
const transformPlaylistsToM3U8 = (destination, ext) => ({ [LIBRARY]: library }) => {
  if (library) {
...
export default function (filePath = resolve(userhome(), 'Music/iTunes/iTunes Library.xml'), destination = resolve('./m3u'), ext = 'm3u8') {
